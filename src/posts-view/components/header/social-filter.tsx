...
function PlatformAvatar({
  ...
  return (
    <span
      className="bg-background ring-muted flex shrink-0 items-center justify-center rounded-full ring-2"
    ...
    <div className="flex items-center">
      {SOCIAL_PLATFORM.map((platform, i) => {
        ...
        return (
          <span
            key={platform.name}
            className="bg-background flex shrink-0 items-center justify-center rounded-full border-1"
            style={{
              width: size,
              height: size,
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-52 p-1.5" align="start" sideOffset={6}>
        ...
        <button
          ...
          className={`flex w-full cursor-pointer items-center gap-3 rounded-md px-2.5 py-2 text-sm transition-colors ${
