...
function OrgSettings() {
  ...
  const canEdit =
    selectedOrg?.org_role === 'owner' || selectedOrg?.org_role === 'admin'

  const { data: orgDetails, isLoading: detailsLoading } = useQuery({
    queryKey: [QUERY_KEYS.orgDetails],
  ...
  const updateMutation = useMutation({
    mutationFn: (data: { name: string }) => orgApi.updateOrg(data),
    onSuccess: () => {
      toast.success('Organization name updated')
      queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.orgDetails] })
      ...
      <div className="bg-card rounded-lg border p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm capitalize">{org?.status ?? '-'}</p>
          </div>
          <div>
            <label className="text-muted-foreground text-sm font-medium">
              Your Role
            ...
            <p className="mt-1 text-sm">
              {org?.org_role ? formatRole(org.org_role) : '-'}
            </p>
          </div>
          <div>
            <label className="text-muted-foreground text-sm font-medium">
